"""Rewrite lib/*.ts with ASCII-only unicode escapes to avoid UTF-8 corruption."""
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]

PROJECTS = r'''export type Project = {
  id?: string;
  title: string;
  description: string;
...
if __name__ == "__main__":
    path = ROOT / "lib" / "projects.ts"
    path.write_text(PROJECTS, encoding="ascii")
